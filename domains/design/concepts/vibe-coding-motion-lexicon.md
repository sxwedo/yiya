...
description: "把「高级/丝滑」换成可执行词：先拆工具、触发、类型、UX。三篇共 56 词；下篇 37–56 是组件反馈、布局过渡、页面转场与五条 UX。忌整页同一节奏淡入。"
status: draft
domain: design
...
sources:
  ...
  - ../../../raw/articles/Adrian Punk/Vibe Coding 网页动效词典（下篇）：组件反馈、布局过渡与页面转场.md
---

# Definition

**Vibe Coding 网页动效词典**（Adrian Punk 上+中+下，56 词）：用自然语言做网页时，卡点往往不是库，是叫不出名字。只会说「加点高级动效 / 丝滑一点 / 像苹果官网」，模型就把标题、图片、卡片同一节奏淡入，看起来仍像模板。
...
3. **动效类型**：画面怎么动。手感五词：Easing、Duration、Delay、Stagger、Spring。入场八词：Fade、Crossfade、Slide、Scale、Blur / Clip-path / Mask reveal、Wipe。文字：Text / Line / Word reveal；中篇再加 Character reveal、Typewriter、Scramble、Text morphing——短词可用，整段正文会让人等动画。滚动≠视差：滚动是触发，视差是前景背景不同速。中篇 21–36：Parallax / 多层视差、横向滚动、Scroll zoom、Sticky scrollytelling、图片序列、Scroll snap、进度提示；磁吸按钮、Tilt card、光标跟随/聚光灯/拖尾、跟随眼睛、指针响应背景、3D 跟随。下篇 37–51：Accordion、锚定浮层、Modal、Drawer/Bottom sheet、Tab、Carousel、Toast/Snackbar、计数翻牌、Skeleton、加载指示、Success/Error、Icon morphing；空间变化 Layout animation（FLIP）、Shared element、Page transition。点击无法告诉模型菜单从哪来、关时怎么回。手机没有持续 hover，指针类要交代触屏怎么简化。横向滚动在手机可能和返回手势冲突。
4. **UX 规则**（52–56）：Feedback、Affordance、Focus、Reduced motion、Input modality（鼠标/触屏/键盘）。第四层不增加花样，只约束前面所有动效。

滚动触发淡入 ≠ 视差。每个区块最好一个主动作。弹窗突然蹦出、菜单展开像页面坏了、加载时空掉、切页后用户不知道自己在哪——都是下篇要命名的问题。
