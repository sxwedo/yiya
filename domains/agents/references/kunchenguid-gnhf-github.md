---
type: Reference
title: "kunchenguid/gnhf（GitHub）"
description: "kunchenguid/gnhf 书签：睡前把任务交给 agent 过夜跑"
status: draft
domain: agents
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
resource: ../../../raw/bookmarks/github.md
tags: [bookmark, github]
sources: []
related:
  - gnhf
---

# Notes

- **仓库原文**：[打开 raw](../../../raw/bookmarks/github.md)
- **为什么收藏**：睡前把任务交给 agent 过夜跑
- **何时用**：要对齐源码、Issue、Release 或安装入口时从这里进。
- **raw**：收入 `raw/bookmarks/github.md` 列表；原文 URL 仍为 https://github.com/kunchenguid/gnhf

## Related

- [gnhf](../entities/gnhf.md)
