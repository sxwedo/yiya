---
type: Entity
title: "cc-switch"
description: "Claude Code / Codex / OpenCode / OpenClaw 桌面切换助手"
status: draft
domain: agents
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
related:
  - farion1231-cc-switch-github
  - claude
  - codex
  - opencode
sources:
  - ../references/farion1231-cc-switch-github.md
---

# Summary

**cc-switch** 的上游仓库是 [farion1231/cc-switch](https://github.com/farion1231/cc-switch)。Claude Code / Codex / OpenCode / OpenClaw 桌面切换助手。

## Related

- [farion1231/cc-switch（GitHub）](../references/farion1231-cc-switch-github.md)
- [Claude](./claude.md)
- [Codex](./codex.md)
- [OpenCode](./opencode.md)
