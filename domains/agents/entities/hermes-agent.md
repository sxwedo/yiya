---
type: Entity
title: "Hermes Agent"
description: "Nous Research 开源 Agent：随使用生长"
status: draft
domain: agents
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
related:
  - nousresearch-hermes-agent-github
  - alchaincyf-hermes-agent-orange-book-github
  - pi
  - grok-build
  - harness-runtime-layer
  - harness-self-improvement
sources:
  - ../references/nousresearch-hermes-agent-github.md
  - ../references/alchaincyf-hermes-agent-orange-book-github.md
  - ../../../raw/articles/Teknium 🪽/Hermes Agent now comes packaged with Karpathy's LLM-Wiki for creating knowledgebases and.md
  - ../../../raw/articles/岚叔/上手 Hermes Agent 后建议先尝试的十件事情.md
  - ../../../raw/articles/Mr. Buzzoni/－ Most people treat notes, models, and agents as three separate worlds.md
  - ../../../raw/articles/Zaid/The problem with AI agents right now.md
  - ../../../raw/articles/Shilong Liu/A Taxonomy of Self-evolving Agents.md
  - ../../../raw/articles/Andy Stewart/十分钟带你玩转Hermes赫妹Agent.md
  - ../../../raw/articles/AI少年/使用 Hermes Agent 打造私人专属工作流： 自动定时监控 X－Twitter 大佬推文的完整方案。.md
  - ../../../raw/articles/loveabit/Hermes Curator 横空出世！AI Agent 终于会“自我进化”了！.md
---

# Summary

**Hermes Agent** 的上游仓库是 [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)。Nous Research 开源 Agent：随使用生长。同仓相关书签：[alchaincyf/hermes-agent-orange-book](../references/alchaincyf-hermes-agent-orange-book-github.md)。

腾讯技术工程：前台 Loop 做当前任务，Session Archive 留原始经历，Memory / Skills 分载稳定事实与可复用流程，Background Review 决定哪些经验进入未来。PAST-Bench 上固定 MiniMax-M2.7 后，是基线里唯一四类跨会话能力全正向的框架。对照 [Harness 运行时层](../concepts/harness-runtime-layer.md)、[Harness 自改进](../concepts/harness-self-improvement.md)。

## Related

- [NousResearch/hermes-agent（GitHub）](../references/nousresearch-hermes-agent-github.md)
- [alchaincyf/hermes-agent-orange-book（GitHub）](../references/alchaincyf-hermes-agent-orange-book-github.md)
- [Pi](./pi.md)
- [Grok Build](./grok-build.md)
- [Harness 运行时层](../concepts/harness-runtime-layer.md)
- [Harness 自改进](../concepts/harness-self-improvement.md)
