...
- **增加人的 agency**，围绕工作/生活里的问题组织，而不是产品功能清单。鼓励继续练你在乎的技能，防萎缩。
- **心态 > 易过期的操作口诀。** 「描述你的受众」这类具体行为会随模型过时；更稳的是「今天的 AI 是你用过最差的 AI」「按赌注比例核验」。
- **安全有效不止对话那一刻**：先问哪些任务该委派、哪些该留下；对外披露 AI 怎么用过。
- 练习要费力：边学边用 Claude。官方说今天的 Academy 体验是「最死板的一次」，之后会更个性化。
- 学会用 AI 之后，它能加速学任何题目（图解、苏格拉底式追问）。
