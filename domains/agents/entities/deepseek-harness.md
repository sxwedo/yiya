---
type: Entity
title: "DeepSeek Harness"
description: "DeepSeek 官方 harness：一切皆插件"
status: draft
domain: agents
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
related:
  - deepseek-ai-deepseek-harness-github
  - pi
  - grok-build
sources:
  - ../references/deepseek-ai-deepseek-harness-github.md
  - ../../../raw/articles/Russell/万字长文：Deepseek Harness 一文全看懂！！.md
  - ../../../raw/articles/超级个体｜柿子/OpenClaw 保姆级教程（基础篇）.md
---

# Summary

**DeepSeek Harness** 的上游仓库是 [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)。DeepSeek 官方 harness：一切皆插件。

## Related

- [deepseek-ai/deepseek-harness（GitHub）](../references/deepseek-ai-deepseek-harness-github.md)
- [Pi](./pi.md)
- [Grok Build](./grok-build.md)
- [万字长文：Deepseek Harness 一文全看懂！！](../../../raw/articles/Russell/万字长文：Deepseek Harness 一文全看懂！！.md)
- [OpenClaw 保姆级教程（基础篇）](../../../raw/articles/超级个体｜柿子/OpenClaw 保姆级教程（基础篇）.md)
