---
type: Reference
title: "astral-sh/uv（GitHub）"
description: "astral-sh/uv 书签：Rust 写的 Python 包与项目管理器"
status: draft
domain: engineering
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
resource: ../../../raw/bookmarks/github.md
tags: [bookmark, github]
sources: []
related:
  - uv
---

# Notes

- **仓库原文**：[打开 raw](../../../raw/bookmarks/github.md)
- **为什么收藏**：Rust 写的 Python 包与项目管理器
- **何时用**：要对齐源码、Issue、Release 或安装入口时从这里进。
- **raw**：收入 `raw/bookmarks/github.md` 列表；原文 URL 仍为 https://github.com/astral-sh/uv

## Related

- [uv](../entities/uv.md)
