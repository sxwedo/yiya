...
sources:
  ...
  - ../../../raw/articles/孔哥进化录/从 DWD 到 DWS：数仓建模的进阶之路—— 建设步骤、避坑指南、实战案例与优化策略.md
---
...
从 DWD 走到 DWS 的操作纪律（孔哥进化录）：DWD 只标准化可分析业务事件（唯一 `event_code`、原子事实、业务时间、按天分区），**禁止在 DWD 做聚合**。DWS 围绕一个实体回答 1～2 类问题，字段必须可解释、计算可复现、更新可追溯；用增量 + 快照分区，不要全量重建，也不要一张万能宽表。口径文档和双通道一致性（实时/离线）是信任前提。建模的终点是组织共识，不是表的张数。


## Boundaries

不要推翻四层去画八层十层。DWD 禁止聚合。不要一张万能宽表。ADS 不是架构终点，其上补语义，不是拆掉 ODS–DWD。
