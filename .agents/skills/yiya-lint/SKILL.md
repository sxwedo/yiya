...
description: >-
  ...
  broken local links, and raw-manifest orphans. Use when the user asks to 检查,
...
3. **manifest 孤儿**：`raw-manifest.yaml` 中 `domain` 对本域的条目，是否存在对应 `references/` 页；`path` 文件是否还在。
4. **重复实体**：本域 `entities/` 与 `shared/entities/` 标题/别名明显撞车时列出（升格用 `yiya-promote-to-shared`）。
5. **过瘦/过肥**：同一 raw 对应 Reference 过多，或一文下 Concept 明显「集邮」，在报告里提醒（不强制删）。
