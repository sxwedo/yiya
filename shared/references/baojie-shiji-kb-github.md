---
type: Reference
title: "baojie/shiji-kb（GitHub）"
description: "baojie/shiji-kb 书签：《史记》→ 可交互知识图谱与 wiki"
status: draft
domain: shared
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
resource: ../../raw/bookmarks/github.md
tags: [bookmark, github]
sources: []
related:
  - shiji-kb
---

# Notes

- **仓库原文**：[打开 raw](../../raw/bookmarks/github.md)
- **为什么收藏**：《史记》→ 可交互知识图谱与 wiki
- **何时用**：要对齐源码、Issue、Release 或安装入口时从这里进。
- **raw**：收入 `raw/bookmarks/github.md` 列表；原文 URL 仍为 https://github.com/baojie/shiji-kb

## Related

- [史记知识库](../entities/shiji-kb.md)
