---
type: Reference
title: "kepano/obsidian-minimal（GitHub）"
description: "kepano/obsidian-minimal 书签：Obsidian 极简可定制主题"
status: draft
domain: shared
generated: { by: agent:yiya-librarian, at: 2026-09-07T21:30:00Z }
resource: ../../raw/bookmarks/github.md
tags: [bookmark, github]
sources: []
related:
  - obsidian
---

# Notes

- **仓库原文**：[打开 raw](../../raw/bookmarks/github.md)
- **为什么收藏**：Obsidian 极简可定制主题
- **何时用**：要对齐源码、Issue、Release 或安装入口时从这里进。
- **raw**：收入 `raw/bookmarks/github.md` 列表；原文 URL 仍为 https://github.com/kepano/obsidian-minimal

## Related

- [Obsidian](../entities/obsidian.md)
