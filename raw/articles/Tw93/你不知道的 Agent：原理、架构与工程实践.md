...
ingested: "2026-09-07"
date: "Thu Mar 19 13:48:03 +0000 2026"
...
const tool = {
  name: "update\_yuque\_post",
  input\_schema: {
    properties: {
      post\_id: { type: "string" },
      content: { type: "string" },
    },
  },
};
...
const updateTool = betaZodTool({
  ...
  inputSchema: z.object({
    post\_id: z.string().describe("语雀文章 ID，纯数字字符串，如 '12345678'"),
    title: z.string().optional().describe("文章标题，不改时可省略"),
    content\_markdown: z.string().describe("Markdown 格式正文"),
  }),
  run: async (input) =&gt; {  // input 类型自动推导，问题尽量在编译期暴露
    const post = await getPost(input.post\_id);
    if (!post) throw new ToolError("文章 ID 不存在", {
      error\_code: "POST\_NOT\_FOUND",
      suggestion: "请先调用 list\_yuque\_posts 获取有效的 post\_id",
    });
    return await updatePost(input.post\_id, input.title, input.content\_markdown);
  },
});
\`\`\`
...
\`\`\`json
{
  "tasks": \[
    ...
    {"id": "2", "desc": "修改数据库 schema", "status": "in\_progress"},
...
{
  request\_id, from\_agent, to\_agent,
  content,
...
\`\`\`markdown
\# Agent 执行时 emit 事件
on tool\_start: emit { type, tool\_name, input, timestamp }
on tool\_end:   emit { type, tool\_name, result, duration }
on turn\_end:   emit { type, turn\_output }

\# 多路下游订阅，Agent 核心代码不变
agent.on("event") -&gt; write\_to\_logs
agent.on("event") -&gt; update\_ui
agent.on("event") -&gt; send\_to\_eval\_framework
\`\`\`
...
const inbound = { channel, session\_key, content };

// 每个渠道只需实现三个方法
class ChannelAdapter {
  start() {}
  stop() {}
  send(session\_key, text) {}
}
\`\`\`
...
\`\`\`typescript
const AUTHORIZED\_USERS = new Set(\["user\_id\_tang", "user\_id\_other"\]);

async function handleMessage(msg: InboundMessage): Promise&lt;void&gt; {
  if (!AUTHORIZED\_USERS.has(msg.userId)) {
...
function wrapUntrustedContent(source: string, content: string): string {
  return \[
    \`&lt;untrusted\_content source="${source}"&gt;\`,
...
*Fri Mar 20 07:06:47 +0000 2026*

@HiTw93 收藏

### 2 @Lonely__MH (Lonely)

*Thu Mar 19 14:00:27 +0000 2026*

@HiTw93 这篇 Agent 工程实践长文是近期读过最扎实的一篇！

先收藏，再点赞👍👍👍

### 3 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:01:01 +0000 2026*

@Lonely\_\_MH 谢谢喜欢 一定要看完

### 4 @LZRationalnvest (李志 | Rational Investing)

*Thu Mar 19 14:07:57 +0000 2026*

@HiTw93 很扎实 可以作为我面试的学习资料了😂

### 5 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:11:14 +0000 2026*

@LZRationalnvest 哈哈

### 6 @yi_xin32482 (图解山河)

*Thu Mar 19 14:32:52 +0000 2026*

@HiTw93 深度好文！
不是又来一篇“Agent 要改变世界”，而是把原理、架构、工程坑放一张桌子上聊。 
今年做 Agent，真正拉开差距的不是 demo 漂不漂亮，是你能不能把上下文、工具调用、失败恢复这些脏活收住。 长文值得存。

### 7 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:34:32 +0000 2026*

@yi\_xin32482 谢谢喜欢

### 8 @sarvneo (Sarv)

*Thu Mar 19 15:04:33 +0000 2026*

@HiTw93 太强了！最近在写 agent runtime 工具，文章中很多点都是工具落地过程中实际遇到的问题

### 9 @HiTw93 (Tw93) (Author)

*Thu Mar 19 15:06:10 +0000 2026*

@sarvneo 我会把团队一些落地过程中碰到坑也融合进去了，更像白皮书

### 10 @heycoollee (Lee)

*Sat Mar 21 04:46:05 +0000 2026*

@HiTw93 请教这种好看的流程图是用什么工具制作的

### 11 @HiTw93 (Tw93) (Author)

*Sun Mar 22 01:06:43 +0000 2026*

@heycoollee 当然是claude

### 12 @kai_jsk (Kai)

*Fri Mar 20 01:11:09 +0000 2026*

@HiTw93 93兄是踩了多少坑才能写这么全面的Agent的原理😛。非常棒的文章，正好最近我在重构我的agent集群，思路变大大滴清晰，感谢🙏

### 13 @HiTw93 (Tw93) (Author)

*Fri Mar 20 01:11:33 +0000 2026*

@kai\_jsk 感谢喜欢！

### 14 @yanglukuan (谦)

*Thu Mar 19 14:43:54 +0000 2026*

@HiTw93 文章内容挺好，先收藏，要是能把参考文献也贴一下就更好了

### 15 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:47:26 +0000 2026*

@yanglukuan 已经快速补充到文章最后面，放回复看了看超链接markdown效果很差

### 16 @sailfishcc1 (橙子🍊啊)

*Thu Mar 19 14:24:10 +0000 2026*

@HiTw93 @HUABANG1 这篇文章是用 Agent 来写的吗？想了解一下图文混排是怎么做的呀？

### 17 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:34:23 +0000 2026*

@sailfishcc1 @HUABANG1 自己手工一个一个copy排
不太喜欢用ai操作这个 容易封号 没必要了

### 18 @frankekn (Frank Y)

*Thu Mar 19 14:42:32 +0000 2026*

@HiTw93 你的文 跟作品 真的都高優質！

### 19 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:43:23 +0000 2026*

@frankekn 强迫症患者 不太想写垃圾

### 20 @Sylvan_Kernel (Sylvan丶🌿)

*Thu Mar 19 14:01:11 +0000 2026*

@HiTw93 这篇真牛逼。

### 21 @HiTw93 (Tw93) (Author)

*Thu Mar 19 14:02:31 +0000 2026*

@Sylvan\_Lex 惭愧 自己的学习笔记

### 22 @congxiaobai1233 (moy)

*Sat Mar 21 16:54:28 +0000 2026*

@HiTw93 写的体系太完整了，佬是如何获取这些信息的哇，求教

### 23 @HiTw93 (Tw93) (Author)

*Sun Mar 22 01:07:07 +0000 2026*

@congxiaobai1233 之前自己看到过的各种文章

### 24 @zhanlin990410 (Yan)

*Thu Mar 26 09:11:27 +0000 2026*

@HiTw93 哥 非程序员背景 读完这篇你觉得能获得最大的收获是什么

### 25 @HiTw93 (Tw93) (Author)

*Thu Mar 26 10:23:03 +0000 2026*

@zhanlin990410 是什么

### 26 @sunbird89630 (How.Wang)

*Mon Mar 23 11:55:21 +0000 2026*

@HiTw93 如何理解这个公式: O(n2)O(n2) ?

### 27 @HiTw93 (Tw93) (Author)

*Mon Mar 23 12:02:19 +0000 2026*

@sunbird89630 应该是渲染错误了，其实就是O(n2）
